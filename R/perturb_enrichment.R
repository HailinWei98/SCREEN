...
CalculatePerturbEnrichment <- function(mtx_dir, sg_dir, NTC = "NTC", prefix = "./", label = "", 
                                       plot.save = TRUE, table.save = TRUE, top = NULL){
    ...
    if (is.null(top)) {
        p1 <- pheatmap(cluster_pro_plot, treeheight_col = F, treeheight_row = F, main = "Perturbations Enrichment",
                       colorRampPalette(colors = c("grey49", "coral1"))(length(bk)),
        ...
        p1 <- pheatmap(cluster_pro_plot, treeheight_col = F, treeheight_row = F, main = "Perturbations Enrichment",
                       colorRampPalette(colors = c("grey", "coral1"))(length(bk)),
                       legend_breaks = lg_bk, breaks = bk, show_rownames = T, 
                       show_colnames = T)
    }
